...
import { useCreatePlateRaceMutation } from '@/hooks/mutations/useCreatePlateRaceMutation';
import { useState } from 'react';
import { Plus, X } from 'lucide-react';
import { useAppForm } from '@/hooks/useAppForm';
import { CreatePlateRaceByUserIdFormSchema } from '@katieeitak/shared';
import { toast } from 'sonner';

export function NewPlateRaceDialog() {
  const { mutate } = useCreatePlateRaceMutation();
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value: formValues }) => {
      mutate(
        ...
        },
        {
          onSuccess: () => {
            setOpen(false);
            toast.success('New plate race created!');
            form.reset();
          },
        },
      );
    ...
    validators: {
      onSubmit: CreatePlateRaceByUserIdFormSchema,
    },
  });
  ...
  return (
    ...
    >
      <Dialog.Trigger className="flex items-center justify-center bg-mainbtn text-white py-2 px-4 rounded-sm">
        <Plus size={21} />
        <span className="font-semibold text-sm">Create a plate race</span>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 w-96 max-w-[calc(100vw-4.5rem)] max-h-[90dvh] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg bg-gray-50 text-gray-900 outline-1 outline-gray-200 transition-all duration-150 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0 dark:outline-gray-300">
          <div className="flex flex-col px-6 py-4 shrink-0">
            <div className="flex justify-between items-center pb-4">
              <h2 className="text-md font-semibold">Create new plate race</h2>
              <div>
                <Button
                  onClick={() => {
                    setOpen(false);
            ...
            <div className="flex flex-col pt-4 pb-6">
              <form>
                <form.AppField
                  name="title"
                  children={(field) => (
                    <field.TextField label="Plate race title" placeholder="e.g. 2025 plates" />
                  )}
                />
              </form>
            </div>
